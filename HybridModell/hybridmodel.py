...
# Flash attention import with fallback
try:
...
class Stage(nn.Module):
    def __init__(self, args, depth=0):
        ...
        # Handle input dimensions: S3DIS now has 7 dims, ScanNetV2 has 10 dims
        ...
        if first:
            ...
            nbr_in_dim = target_dim
            unified_dim = target_dim
        else:
            nbr_in_dim = 3
            unified_dim = 3
        ...
        # No input projection needed anymore since both datasets have 10 dims
            
        self.nbr_embed = nn.Sequential(
            nn.Linear(unified_dim if first else nbr_in_dim, nbr_hid_dim//2, bias=False),
    ...
    def forward(self, x, xyz, prev_knn, indices, pts_list):
        ...
        if self.first:
            # Concatenate spatial and feature information
            # Input now flexible: 7 or 10 dims after projection
            input_dim = x.shape[-1]
            nbr = torch.cat([nbr, x[knn]], dim=-1)  # [N, k, 3+input_dim]
            nbr = nbr.view(-1, 3 + input_dim)  # [N*k, 3+input_dim]
        else:
            nbr = nbr.view(-1, 3)  # [N*k, 3] - only spatial differences
...
class DelaEncoder(nn.Module):
    """
    Encoder part of DeLA model for feature extraction
    """
    ...
    def forward(self, xyz, x, indices, pts_list=None):
        ...
        Args:
            ...
            pts_list: Points per scene
            
        Returns:
            encoded_features: Encoded features
            closs: Consistency loss (if training)
        """
        indices = indices[:]
        encoded_features, closs = self.stage(x, xyz, None, indices, pts_list)
        
        if self.training:
            return encoded_features, closs
        return encoded_features

class DelaDecoder(nn.Module):
    """
    Decoder part of DeLA model for classification/segmentation
    """
    def __init__(self, args):
        super().__init__()
...
class DelaSemSeg(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.backbone = DelaEncoder(args)  # For compatibility with pretrainer
        self.encoder = self.backbone  # Alias for clarity
        self.decoder = DelaDecoder(args)
    ...
    def forward(self, xyz, x, indices, pts_list=None):
        ...
        # Encode features
        if self.training:
            encoded_features, closs = self.encoder(xyz, x, indices, pts_list)
            # Decode to predictions
            predictions = self.decoder(encoded_features)
            return predictions, closs
        else:
            encoded_features = self.encoder(xyz, x, indices, pts_list)
            # Decode to predictions
            predictions = self.decoder(encoded_features)
            return predictions
